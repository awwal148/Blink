  ...
  return (
    <section className={`w-[100%] h-[17rem] gap-3 max-sm:gap-0 lg:pb-4 ${checkCart ? 'gap-0 h-full' : ""}`}>
        ...
        <div className="">
            ...
            {gender.map((index) => (
                <p key={index.men} className={`text-[1.4rem] max-sm:text-[1rem] text-coral-red font-semibold ${checkCart ? 'text-[1rem]' : ""}`
            }>{index.men}</p>
            ))}
        ...
        <div className={`flex flex-col justify-normal items-end max-sm:items-center ${checkCart ? 'max-sm:items-center lg:items-center' : ""}`}>
            <p className={`font-semibold text-[1.5rem] max-sm:text-[1.1rem]${checkCart ? 'text-[1.1rem]' : ""}`}>£{Price * cartItems[rating]}</p>
            </div>
        </div>
    </section>
  )
